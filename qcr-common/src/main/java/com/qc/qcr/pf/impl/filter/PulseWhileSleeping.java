...
public class PulseWhileSleeping extends AbstractActiveFilter<LogData<Integer>, Integer> {

	Integer sum = 0;
	Integer count = 0;
	
	public PulseWhileSleeping(DataPipe<LogData<Integer>> inputPipe, DataPipe<Integer> outputPipe) {
	...
	public void process(DataPipe<LogData<Integer>> inputPipe, DataPipe<Integer> outputPipe) {
		...
		else if(inData.getType() == LogDataType.HEART_RATE && (inData.getDate().get(Calendar.HOUR_OF_DAY) >= 22 && inData.getDate().get(Calendar.HOUR_OF_DAY) <= 8)) {
			sum += inData.getValue();
			count++;
		}
	}
